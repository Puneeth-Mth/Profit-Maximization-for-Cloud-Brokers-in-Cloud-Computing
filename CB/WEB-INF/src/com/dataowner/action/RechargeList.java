	...
	public void doGet(HttpServletRequest request,HttpServletResponse response)throws IOException
	{
		...
		try
		{
			...
			if(submit.equals("get"))
			{
				...
				rd=request.getRequestDispatcher("/res/jsp/owner/recharge.jsp?no=1");
				...
				if(Utility.parse1(request.getParameter("add1")).equals("YES"))
				{
					...
					if(!result)
					{
						...
						if(result)
						{
							rs = AdminDAO.getrechargeDetails();
							request.setAttribute("rs", rs);
							rd=request.getRequestDispatcher("/res/jsp/owner/recharge.jsp?no=1&no1=1");
							rd.forward(request, response);
						}
						else
						{
							rd=request.getRequestDispatcher("/res/jsp/owner/recharge.jsp?no=2&no1=2");
							rd.forward(request, response);
						}
					...
					{
						rd=request.getRequestDispatcher("/res/jsp/owner/recharge.jsp?no=2&no1=1");
						rd.forward(request, response);
					}
				}
				else
				{
					rd=request.getRequestDispatcher("/res/jsp/owner/recharge.jsp?no=2");
					rd.forward(request, response);
				}
			}
			...
			if(submit.equals("Delete"))
			{
				...
				if(chk==null)
				{
					rs = AdminDAO.getrechargeDetails();
					request.setAttribute("rs", rs);
					rd=request.getRequestDispatcher("/res/jsp/owner/recharge.jsp?no=1&no1=3");
					rd.forward(request, response);
				}
				else
				{
					...
					rs = AdminDAO.getrechargeDetails();
					request.setAttribute("rs", rs);
					rd=request.getRequestDispatcher("/res/jsp/owner/recharge.jsp?no=1&no1=5");
					rd.forward(request, response);
				}
			}
		}
